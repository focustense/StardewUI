...
public ref struct DocumentReader(Lexer lexer)
{
    ...
    public DocumentReader(ReadOnlySpan<char> text) : this(new Lexer(text)) { }
    ...
    public bool NextAttribute()
    {
        ...
        switch (lexer.Current.Type)
        {
            case TokenType.Name:
                ...
                var valueType = lexer.Current.Type == TokenType.BindingStart ? AttributeValueType.Binding : AttributeValueType.Literal;
...
public class ParserException(string message, int position)
    : Exception(message)
{
