...
{
    public IEnumerable<IPropertyDescriptor> Properties => propertiesByName.Values;

    public Type TargetType { get; }

    private static readonly Dictionary<Type, ReflectionObjectDescriptor> cache = [];
    private readonly IReadOnlyDictionary<string, IPropertyDescriptor> propertiesByName;
    ...
    protected static T CreateForType<T>(
        Type type,
        Func<Type, IReadOnlyDictionary<string, IPropertyDescriptor>, T> create
    )
        where T : ReflectionObjectDescriptor
    {
        var propertiesByName = type.GetProperties(BindingFlags.Instance | BindingFlags.Public)
            ...
            .Select(ReflectionPropertyDescriptor.FromPropertyInfo)
    ...
    protected ReflectionObjectDescriptor(Type type, IReadOnlyDictionary<string, IPropertyDescriptor> propertiesByName)
    {
        TargetType = type;
